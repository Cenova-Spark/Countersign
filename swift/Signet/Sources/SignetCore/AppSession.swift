...
/// The Mac's answer path, as the shared screen sees it.
///
/// `SignetUI` knows nothing about daemons or unix sockets — it takes these six
/// verbs and a noun. The iPhone app will supply its own conformance that posts
/// to the relay instead.
extension AppSession: ApprovalActions {
    public var deviceNoun: String { "Mac" }
}
