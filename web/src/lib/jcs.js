 ...
 *
 * A direct port of the TypeScript SDK's `jcs.ts`, which is itself a port of
 * `countersign-verify`'s `jcs.rs`. Canonicalization is where
...
export function canonicalizeText(json) {
  ...
  try {
    ...
    throw new JcsError(`malformed JSON: ${e.message}`, "malformed");
  }
 ...
 * `JSON.parse`.
 */
export function findDuplicateKey(json) {
  const scopes = []; // null marks an array scope
  let i = 0;
  let expectKey = false;
...
function readString(json, start) {
  let i = start + 1;
  let raw = '"';
  while (i < json.length) {
    const char = json[i];
